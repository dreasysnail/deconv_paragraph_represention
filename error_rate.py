...
def cal_cer(y_total, y_hat_total):
    error = 0
    length = 0
    for y, y_hat in zip(y_total, y_hat_total):
        y_len = len(''.join(y.split()))
        error += min(y_len/float((len(y))), wer(y, y_hat) )* len(y)
        length += y_len
    return error/length
...
def prepare_for_cer(sentence, ixtoword):
